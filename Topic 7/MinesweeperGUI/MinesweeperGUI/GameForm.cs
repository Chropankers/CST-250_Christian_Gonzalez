    ...
    public partial class GameForm : Form
    {
            ...
            for (int row = 0; row < gridSize; row++)
            {
                for (int col = 0; col < gridSize; col++)
                {
                    Button btn = new Button
                    {
                        ...
                        Location = new Point(startX + (buttonSize * col), startY + (buttonSize * row)),
        ...
        private void HandleCellClick(int row, int col, Button btn)
        {
            ...
            if (cell.Live)
            {
                ...
                stopwatch.Stop();
                timer.Stop();
                SaveScore("Player", difficultyLevel, stopwatch.Elapsed, false);
                ShowHighScores();
                return;
            }
            ...
            if (board.CheckWinCondition())
            {
                ...
                SaveScore(playerInitials, difficultyLevel, stopwatch.Elapsed, true);
                ShowHighScores();
        ...
        private void BtnDebugWin_Click(object sender, EventArgs e)
        {
            for (int row = 0; row < gridSize; row++)
            {
                for (int col = 0; col < gridSize; col++)  // Fixed loop to ensure all columns are iterated
                {
                    Cell cell = board.Grid[row, col];
                    if (!cell.Live)
                    {
            ...
            if (board.CheckWinCondition())
            {
                ...
                stopwatch.Stop();
                timer.Stop();
                SaveScore("DBG", difficultyLevel, stopwatch.Elapsed, true); // Save debug score as win
                ShowHighScores();
                MessageBox.Show($"Congratulations! You've cleared all non-mine cells in {stopwatch.Elapsed.TotalSeconds} seconds.");
            }
        }

        private void SaveScore(string playerInitials, int difficultyLevel, TimeSpan timeElapsed, bool won)
        {
            var playerStats = new PlayerStats(playerInitials, difficultyLevel, timeElapsed, won);
        ...
        {
            var highScoreForm = new HighScoreForm(allScores, difficultyLevel);
            highScoreForm.ShowDialog(); // Use ShowDialog to make sure it appears modally
        }

        private void GameForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            Application.Exit(); // Ensure the entire application exits when the GameForm is closed
        }
    }
